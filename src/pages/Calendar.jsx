...
function eventBlockGlassStyle(c, extra) {
  return Object.assign({
    ...
    boxShadow: "0 4px 16px " + c + "18, inset 0 1px 0 rgba(255,255,255,0.06)",
  }, extra || {});
}
...
export default function Calendar() {
  ...
  return (
    <div ref={pageRef} className={"cal-page" + (isMobile ? " cal-page--mobile" : "")} data-scrollable style={{ minHeight: "100vh", height: "100vh", background: "linear-gradient(160deg, #0A0A0F 0%, #0D1218 45%, #0A0A0F 100%)", color: "#fff", fontFamily: "'IBM Plex Sans', sans-serif", position: "relative", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <main className="cal-shell">
        ...
        <section className="cal-grid-area">
          {!isMobile && (
          <div style={{ display: "flex", alignItems: "center", marginBottom: 12, gap: 10, flexShrink: 0 }}>
            <button type="button" onClick={function() { setSidebarOpen(!sidebarOpen); }} title={sidebarOpen ? "Recolher menu lateral" : "Mostrar menu lateral"} aria-label={sidebarOpen ? "Recolher menu lateral" : "Mostrar menu lateral"}
              style={{ width: 32, height: 32, flexShrink: 0, borderRadius: 9, border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.03)", color: "rgba(255,255,255,0.55)", cursor: "pointer", fontSize: 14, lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'JetBrains Mono',monospace" }}>
              {sidebarOpen ? "⟨" : "⟩"}
            </button>
            <h2 style={{ flex: 1, margin: 0, fontSize: "clamp(14px,3.5vw,20px)", fontFamily: "'JetBrains Mono',monospace", fontWeight: 500, textTransform: "capitalize", color: "rgba(255,255,255,0.9)", minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{weekLabel}</h2>
            <button type="button" onClick={toggleFullscreen} title={isFullscreen ? "Sair de ecrã inteiro" : "Ecrã inteiro"} aria-label={isFullscreen ? "Sair de ecrã inteiro" : "Ecrã inteiro"}
              style={{ width: 32, height: 32, flexShrink: 0, borderRadius: 9, border: "1px solid " + (isFullscreen ? ACCENT + "45" : "rgba(255,255,255,0.08)"), background: isFullscreen ? ACCENT + "14" : "rgba(255,255,255,0.03)", color: isFullscreen ? ACCENT : "rgba(255,255,255,0.55)", cursor: "pointer", fontSize: 14, lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
              ⛶
            </button>
          </div>
          )}
