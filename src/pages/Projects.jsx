...
var PROJ_CSS = [
  ...
  ".pj-search:focus-within svg{color:#A0A0A8}",
  ".pj-search input{width:100%;padding:9px 12px 9px 34px;border-radius:10px;border:1px solid rgba(255,255,255,0.07);background:#141416;color:#EDEDEF;font-size:13px;line-height:1.5;outline:none;font-family:inherit;transition:border-color var(--dur) var(--ease),background-color var(--dur) var(--ease)}",
  ".pj-search input:focus{border-color:rgba(255,255,255,0.14);background:#1A1A1D}",
  ".pj-viewtog{display:flex;border-radius:10px;border:1px solid rgba(255,255,255,0.07);overflow:hidden}",
  ".pj-viewtog button{width:34px;height:34px;display:flex;align-items:center;justify-content:center;background:#141416;border:none;color:#6E6E76;cursor:pointer;transition:background-color var(--dur) var(--ease),color var(--dur) var(--ease)}",
  ".pj-viewtog button.on{background:#1A1A1D;color:#EDEDEF}",
  ".pj-top-actions{display:flex;align-items:center;gap:12px;margin-left:auto}",
  ".pj-new{padding:9px 16px;border-radius:10px;border:none;background:#EDEDEF;color:#09090B;font-family:'JetBrains Mono',monospace;font-size:11px;font-weight:500;letter-spacing:.4px;cursor:pointer;white-space:nowrap}",
  ".pj-preset{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:4px}",
  ".pj-preset button{flex:1;min-width:90px;padding:10px 8px;border-radius:10px;border:1px solid rgba(255,255,255,0.07);background:#0E0E10;color:#A0A0A8;font-size:11px;font-family:'JetBrains Mono',monospace;cursor:pointer;transition:background-color var(--dur) var(--ease),border-color var(--dur) var(--ease),color var(--dur) var(--ease)}",
  ".pj-preset button.on{border-color:rgba(255,255,255,0.14);background:#1A1A1D;color:#EDEDEF}",
  ".pj-scroll{position:relative;z-index:1;flex:1;min-height:0;overflow-y:auto;padding:0 20px 28px;-webkit-overflow-scrolling:touch}",
  ".pj-body{width:100%;max-width:920px;margin:0 auto;padding-top:20px}",
  ".pj-body.dense{max-width:1180px}",
  ".pj-hero{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;flex-wrap:wrap;margin-bottom:20px;padding:18px 20px;border-radius:14px;border:1px solid rgba(255,255,255,0.07);background:#141416;position:relative;overflow:hidden}",
  ".pj-hero h2{margin:0;font-family:'JetBrains Mono',monospace;font-size:clamp(17px,3vw,22px);font-weight:500;letter-spacing:-.01em;color:#EDEDEF}",
  ".pj-hero p{margin:8px 0 0;font-size:12.5px;color:#A0A0A8;line-height:1.55;max-width:420px}",
  ".pj-chips{display:flex;gap:8px;flex-wrap:wrap;align-items:center}",
  ".pj-chip{padding:7px 12px;border-radius:999px;border:1px solid rgba(255,255,255,0.07);background:#0E0E10;font-size:10.5px;font-family:'JetBrains Mono',monospace;color:#A0A0A8;white-space:nowrap}",
  ".pj-chip strong{color:#EDEDEF;font-weight:500}",
  ".pj-sech{margin:0 0 12px;font-size:10px;font-family:'JetBrains Mono',monospace;font-weight:500;color:#6E6E76;letter-spacing:1.6px;display:flex;align-items:center;gap:10px}",
  ".pj-sech::after{content:'';flex:1;height:1px;background:rgba(255,255,255,0.06)}",
  ".pj-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:14px;align-content:start;justify-content:center}",
  ".pj-grid.sparse{grid-template-columns:repeat(auto-fit,minmax(280px,420px));justify-content:center;gap:16px}",
  ".pj-grid.sparse .pj-card{border-radius:14px}",
  ".pj-grid.sparse .pj-card-in{min-height:156px;padding:18px 18px 16px;gap:12px}",
  ...
  ".pj-grid.sparse .pj-card-desc{font-size:13px;-webkit-line-clamp:3}",
  ".pj-list{display:flex;flex-direction:column;gap:10px;max-width:760px;margin:0 auto}",
  ".pj-card{position:relative;border-radius:14px;cursor:pointer;overflow:hidden;border:1px solid rgba(255,255,255,0.07);background:#141416;box-shadow:none;transition:background-color var(--dur) var(--ease),border-color var(--dur) var(--ease),transform var(--dur) var(--ease),box-shadow var(--dur) var(--ease)}",
  ".pj-card:hover{background:#1C1C20;border-color:rgba(255,255,255,0.14)}",
  ".pj-card-bar{height:2px;background:var(--pc);opacity:.7}",
  ...
  ".pj-pin{position:absolute;top:10px;left:10px;font-size:12px;color:#C4A57C;z-index:1;}",
  ".pj-addcard{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;min-height:156px;border-radius:14px;border:1px dashed rgba(255,255,255,0.14);background:transparent;cursor:pointer;transition:background-color var(--dur) var(--ease),border-color var(--dur) var(--ease),color var(--dur) var(--ease);color:#6E6E76}",
  ".pj-addcard:hover{border-color:rgba(255,255,255,0.14);background:#1A1A1D;color:#EDEDEF}",
  ".pj-addcard span:first-child{font-size:24px;line-height:1;opacity:.7}",
  ".pj-addcard span:last-child{font-family:'JetBrains Mono',monospace;font-size:11px;letter-spacing:.6px}",
  ".pj-rowcard{display:flex;align-items:center;gap:14px;padding:12px 16px;border-radius:12px;border:1px solid rgba(255,255,255,0.07);background:#141416;cursor:pointer;box-shadow:none;transition:background-color var(--dur) var(--ease),border-color var(--dur) var(--ease),transform var(--dur) var(--ease),box-shadow var(--dur) var(--ease);position:relative;overflow:hidden}",
  ".pj-rowcard:hover{background:#1C1C20;border-color:rgba(255,255,255,0.14)}",
  ".pj-rowcard::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3px;background:var(--pc);opacity:.6}",
  ".pj-empty{display:flex;flex-direction:column;align-items:center;justify-content:center;padding:48px 24px;border-radius:14px;border:1px dashed rgba(255,255,255,0.14);background:transparent;text-align:center;max-width:480px;margin:40px auto 0}",
  ...
  ".pj-modal{width:min(450px,100%);border-radius:16px;border:1px solid rgba(255,255,255,0.14);background:#141416;box-shadow:0 24px 60px rgba(0,0,0,0.5);padding:20px;animation:pjFade var(--dur-slow) var(--ease)}",
  ...
  ".pj-pal button{width:28px;height:28px;border-radius:9px;border:2px solid transparent;cursor:pointer;padding:2px;transition:transform var(--dur-fast) var(--ease),border-color var(--dur) var(--ease)}",
  ".pj-ghost{padding:9px 13px;border-radius:10px;border:1px solid rgba(255,255,255,0.07);background:#141416;color:#A0A0A8;font-size:11.5px;cursor:pointer;transition:background-color var(--dur) var(--ease),border-color var(--dur) var(--ease),color var(--dur) var(--ease)}",
  ".pj-ghost:hover{background:#1A1A1D;border-color:rgba(255,255,255,0.14);color:#EDEDEF}",
  "@media(hover:hover){.pj-card:hover{background:#1A1A1D;border-color:rgba(255,255,255,0.14);transform:translateY(-2px);box-shadow:0 10px 28px rgba(0,0,0,0.45)}.pj-rowcard:hover{background:#1A1A1D;border-color:rgba(255,255,255,0.14);transform:translateY(-2px);box-shadow:0 10px 28px rgba(0,0,0,0.45)}.pj-addcard:hover{transform:translateY(-2px)}.pj-card:hover .pj-act,.pj-rowcard:hover .pj-act{opacity:1}.pj-new:hover{background:#232326;border-color:rgba(255,255,255,0.22)}.pj-viewtog button:hover{background:#1A1A1D;color:#EDEDEF}.pj-preset button:hover{background:#1A1A1D;border-color:rgba(255,255,255,0.14);color:#EDEDEF}.pj-mods-grid button:hover{background:#1A1A1D;border-color:rgba(255,255,255,0.14);color:#A0A0A8}.pj-pal button:hover{transform:scale(1.12)}}",
  "@media(hover:none){.pj-act{opacity:1}.pj-card:hover,.pj-rowcard:hover,.pj-addcard:hover{transform:none;box-shadow:none}}",
...
function RoiPill(props) {
  ...
  return (
    <span className="pj-roi" style={{
      color: pos ? "#8FB39B" : "#C08C8C",
      background: pos ? "rgba(143,179,155,0.1)" : "rgba(192,140,140,0.1)",
      border: "1px solid " + (pos ? "rgba(143,179,155,0.25)" : "rgba(192,140,140,0.25)"),
    }}>
        ...
        <div className="pj-card-foot">
          {st.total > 0 ? (
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono',monospace", color: "#A0A0A8" }}>{st.pending} por fazer</span>
            </div>
          ) : (
            <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono',monospace", color: "#6E6E76" }}>Sem tarefas</span>
          )}
...
function ProjectRow(props) {
  ...
  return (
    <div className="pj-rowcard" style={{ "--pc": meta.color, "--pcb": meta.color + "55", animationDelay: (props.index * 0.03) + "s" }} onClick={props.onOpen}>
      <div className="pj-card-ic" style={{ width: 38, height: 38, borderRadius: 11, background: meta.color + "1a", color: meta.color, fontSize: 17 }}>{meta.icon}</div>
      ...
      {st.total > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          <ProgressRing pct={donePct} color={meta.color} />
          <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono',monospace", color: "#A0A0A8", width: 60 }}>{st.pending} restam</span>
        </div>
      )}
      {st.hasInvestments && <RoiPill roi={st.roi} />}
        ...
        <div className={"pj-body" + (isSparse ? "" : " dense")}>
          ...
          {filtered.length === 0 ? (
            <div className="pj-empty">
              <p style={{ margin: "0 0 8px", fontFamily: "'JetBrains Mono',monospace", fontSize: 15, fontWeight: 500, color: ACCENT }}>Nenhum projeto encontrado</p>
              <p style={{ margin: "0 0 20px", fontSize: 12.5, lineHeight: 1.55, color: "#A0A0A8" }}>
            ...
            <>
              ...
              <div className={view === "list" ? "pj-list" : "pj-grid"} style={{ marginBottom: 24 }} data-stagger>
                {pinnedItems.map(function(p, idx) { return renderItem(p, idx); })}
              </div>
              <p className="pj-sech">TODOS OS PROJETOS</p>
              <div className={view === "list" ? "pj-list" : "pj-grid"} data-stagger>
                {otherItems.map(function(p, idx) { return renderItem(p, idx); })}
              </div>
            </>
          ) : (
            <div className={(view === "list" ? "pj-list" : "pj-grid") + (isSparse && view !== "list" ? " sparse" : "")} data-stagger>
        ...
        <div className="pj-mbk" onClick={function(e) { if (e.target === e.currentTarget) setModalOpen(false); }}>
          <div className="pj-modal" onClick={function(e) { e.stopPropagation(); }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <p style={{ margin: 0, fontSize: 10, fontFamily: "'JetBrains Mono',monospace", color: ACCENT, letterSpacing: 1.6 }}>NOVO PROJETO</p>
              <button type="button" className="pj-ghost" onClick={function() { setModalOpen(false); }} style={{ padding: "6px 11px" }}>×</button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <textarea value={description} onChange={function(e) { setDescription(e.target.value); }} placeholder="Descrição — ex: Resale sneakers, SaaS, hardware..."
                rows={2} style={Object.assign({}, inputStyle(), { resize: "none", lineHeight: 1.55, fontSize: 13.5 })} />
              <div>
              ...
              <div>
                ...
                <div className="pj-pal">
                  {PALETTE.map(function(c) {
                    return <button type="button" key={c} onClick={function() { setColor(c); }} style={{ background: c + "33", borderColor: color === c ? c : "transparent" }} title={c}>
                ...
                <div className="pj-mods-grid">
                  {synapseStore.MODULE_META.map(function(m) {
                    return <button type="button" key={m.id} className={modules[m.id] ? "on" : ""} onClick={function() { toggleModule(m.id); }} title={m.desc}>
                      <div style={{ fontSize: 13, marginBottom: 4 }}>{MODULE_ICONS[m.id]}</div>{m.label}
                    </button>;
                  })}
                </div>
              </div>
              <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                <button type="button" className="pj-new" style={{ flex: 1, padding: "11px 16px", textAlign: "center", display: "block" }} onClick={createProject}>Criar e abrir</button>
...
function inputStyle() {
  return { width: "100%", background: "#0E0E10", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, color: "#EDEDEF", padding: "11px 13px", fontSize: 14, lineHeight: 1.5, outline: "none", fontFamily: "inherit", boxSizing: "border-box" };
}
function lblMini() {
  return { margin: "0 0 8px", fontSize: 10, fontFamily: "'JetBrains Mono',monospace", fontWeight: 500, color: "#6E6E76", letterSpacing: 1.6 };
}
